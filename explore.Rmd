...
lm1 = lm(ARR_DELAY~.-CRS_ELAPSED_TIME-AIR_TIME-MFR.Actual-MFR.MDL.CODE-DAY_OF_WEEK, data=train2019)
...
gam.train2019 = gam.train2019[,c(1,2,5,6,9,12:23)]

gam.start  = gam(ARR_DELAY~., data=gam.train2019)
air.scope = gam.scope(gam.train2019)
air.scope
air.step = step.Gam(gam.start, air.scope, direction="both",trace=0) # this isnt working...idk why
summary(air.step) 


anova(gam.start,air.step)

```



## 3h. Classification Tree

```{r}
library(tree)
train2019$ARR.BIN = ifelse(train2019$ARR_DELAY > 0,1,0)

class.tree = tree(as.factor(ARR.BIN)~.-ARR_DELAY-CRS_ELAPSED_TIME-AIR_TIME-MFR.Actual-MFR.MDL.CODE,data= train2019)
plot(class.tree)
