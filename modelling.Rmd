...
# 2.EDA (based on data.for.modelling. Previous is too messy)
...
#synthesis
data.for.modelling[,c(1,2,6,13:20)] %>% gather(-ORIGIN, -DEST, -ARR_DELAY, key="var", value="value") %>% ggplot(aes(x=value, y=ARR_DELAY, color=ORIGIN))+geom_point()+facet_wrap(~var, scales = "free")
...
data.for.modelling = data.for.modelling %>% select(-route)
...
# forward selection; consider lm1 as the biggest
...
gam.train2019 = gam.train2019 %>% select(-CRS_ELAPSED_TIME,-AIR_TIME,-CRS_ARR_TIME_GROUP,-DAY.BIN, -MFR.MDL.CODE)
...
```{r}
library(tree)
train2019$ARR.BIN = ifelse(train2019$ARR_DELAY > 0,1,0)

class.tree = tree(as.factor(ARR.BIN)~.-ARR_DELAY-CRS_ELAPSED_TIME-AIR_TIME-MFR.MDL.CODE,data= train2019)
plot(class.tree)
