...
```{r}
data.for.modelling.full = data.for.modelling #unsubsetted original
data.for.modelling = data.for.modelling %>% filter(ARR_DELAY <= 120) #subsetted 
data.for.modelling.worst = data.for.modelling.full %>% filter(ARR_DELAY > 120) # greater than 120 delay


# transform delay; decide one with Jackson

min.delay = min(data.for.modelling$ARR_DELAY)

transformed.delay = log(data.for.modelling$ARR_DELAY - min.delay + 1)
#transformed.delay = sqrt(data.for.modelling$ARR_DELAY - min.delay+1)

original.delay = data.for.modelling$ARR_DELAY
...
data.for.modelling$ARR_DELAY = original.delay  # in case we need to undo transformation
...
# overall histogram  
univariate + geom_histogram( fill="lightblue", color="black") +labs(x="Arrival Delay")
...
univariate + geom_boxplot(aes(y = MFR.Actual), fill="lightblue")+labs(x="Arrival Delay", y="Aircraft Manufacturer")

# carrier
univariate + geom_boxplot(aes(y = OP_UNIQUE_CARRIER), fill="lightblue")+labs(x="Arrival Delay", y="Airlines")
...
# day of week
univariate +  geom_boxplot(aes(y = DAY_OF_WEEK), fill="lightblue") + labs(x="Arrival Delay", y="Day of Week")

# departure time group
univariate + geom_boxplot(aes(y = CRS_DEP_TIME_GROUP), fill="lightblue") + labs(x="Arrival Delay", y="Departure Time")
...
q.vars = data.for.modelling %>%  gather (-ARR_DELAY, -DEST, -ORIGIN, -DAY_OF_WEEK, -OP_UNIQUE_CARRIER, -MFR.MDL.CODE, -MFR.Actual, -CRS_ARR_TIME_GROUP, -CRS_DEP_TIME_GROUP, -DAY.BIN, key="var", value="value")
...
# 1. Manufacturer
ggplot(q.vars, aes(x=value, y=ARR_DELAY, color=MFR.Actual)) + 
...
ggplot(q.vars, aes(x=value, y=ARR_DELAY)) + 
    facet_wrap(.~var, scales = "free") +
    geom_point(aes(color=MFR.BIN))+
    geom_smooth(aes(fill=MFR.BIN))+
    labs(y="Arrival Delay")
```
...
data.for.modelling = data.for.modelling %>% dplyr::dplyr::select(-route)
...
lm1 = lm(ARR_DELAY~.-CRS_ELAPSED_TIME-AIR_TIME-MFR.MDL.CODE-CRS_ARR_TIME_GROUP-DAY.BIN, data=train2019)
...
lm2= lm(ARR_DELAY~.-CRS_ELAPSED_TIME-AIR_TIME-MFR.Actual-MFR.MDL.CODE-DAY_OF_WEEK-ORIGIN-
...
train.matrix = model.matrix(ARR_DELAY~.-CRS_ELAPSED_TIME-AIR_TIME-MFR.MDL.CODE-CRS_ARR_TIME_GROUP-DAY.BIN, data=train2019)[,-1]

test.matrix = model.matrix(ARR_DELAY~.-CRS_ELAPSED_TIME-AIR_TIME-MFR.MDL.CODE-CRS_ARR_TIME_GROUP-DAY.BIN, data=test2019)[,-1]
...
gam.train2019 = gam.train2019 %>% dplyr::select(-CRS_ELAPSED_TIME,-AIR_TIME,-CRS_ARR_TIME_GROUP,-DAY.BIN, -MFR.MDL.CODE)

gam.train2019 = gam.train2019[,-1] #mfr mdl code wasn't removed in previous line; need to remove it manually
# I decided to keep MFR.Actual, day of month, and day of week
...
air.scope = gam.scope(gam.train2019)
#air.scope
...
#Assumptions for gam
par(mfrow=c(1,3))
plot(residuals(gam.step)~gam.step$fitted.values)
hist(residuals(gam.step))
qqnorm(residuals(gam.step))
qqline(residuals(gam.step))
```
...
class.tree = tree(as.factor(ARR.BIN)~.-ARR_DELAY-CRS_ELAPSED_TIME-AIR_TIME-MFR.MDL.CODE,data= tree.train)
plot(class.tree)
...
## subsetting below 2 hours
```{r}
data.for.modelling2 = data.for.modelling %>% filter (ARR_DELAY <= 120)
hist(log(data.for.modelling2$ARR_DELAY))
